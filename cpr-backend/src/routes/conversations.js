...
router.get('/', async (req, res) => {
  ...
  const convs = await Conversation.find(filter).sort({ lastUpdated: -1 }).limit(200);
  res.json(convs);
});

router.get('/:chatId', async (req, res) => {
  const conv = await Conversation.findOne({ chatId: req.params.chatId });
  res.json(conv || { chatId: req.params.chatId, state: 'idle', pendingData: '' });
});
...
router.put('/:chatId', async (req, res) => {
  ...
  await Conversation.findOneAndUpdate(
    ...
    { $set: { state: state || 'idle', pendingData: pendingData || '', lastUpdated: new Date() } },
    { upsert: true }
  );
  res.json({ ok: true });
});
